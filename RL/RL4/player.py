...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            current_mean , old_mean = np.nanmean(Q) , np.nanmean(Q_old)
            diff = abs(current_mean - old_mean)
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        d_epsilon = self.final_p - self.initial_p
        epsilon_t = self.initial_p + ( d_epsilon * t / self.schedule_timesteps)
        return epsilon_t
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
